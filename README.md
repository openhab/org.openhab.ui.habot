...
- [x] **(Chrome only, uses navigator.credentials)** Authenticate to myopenhab.org without user interaction (OAuth2 or store the basic auth credentials securely?);
...
- [x] **(to reimplement)** Speech support - either using the browser's API or sending the audio stream to openHAB to process locally;
...
- [ ] **(started)** Add an "analyze" buttons underneath chart images to bring up a modal with an interactive chart;
- [x] Cards are currently static, the goal is to make them interactive - render switches as actual switches, dimmer as sliders etc. Also consider the ability to use tags to alter the default control for an item;
- [x] Ability to design custom cards as a JSON object and show them instead of simple controls (in the chat or in a separate web app page, for example to group complete thermostat controls);
- [x] **(in progress, push-a-card feature missing)** Use Web Push notifications to push rich notifications and cards when relevant (e.g. push a card or notification with a webcam image and action buttons when motion was detected, tbd.). If possible show those notifications as messages from HABot in the chat;
